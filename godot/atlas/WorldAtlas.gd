...
func _ready() -> void:
	...
	camera = Camera2D.new()
	camera.enabled = true
	# B4 draw order: raster → land → vectors → flow → hex
	add_child(raster)
	add_child(land)
	add_child(vectors)
	add_child(flow)
...
func _handle_click_at_viewport(vp_pos: Vector2) -> void:
	...
	var river: Dictionary = vectors.pick_river(local, 5.0 / maxf(camera.zoom.x, 0.01))
	if not river.is_empty() and not _prefer_hex_pick:
		inspect_river.emit({
			"id": river.get("id", 0),
	...
	if _prefer_hex_pick or hexes.visible_overlay:
		var hid: int = hexes.hex_at(map_xy)
		inspect_hex.emit(hexes.hex_info(hid))
		return
	inspect_terrain.emit({
		"x": map_xy.x,
		"y": map_xy.y,
		"note": "Sample at map UV; elevation/climate from display mode context",
	})
